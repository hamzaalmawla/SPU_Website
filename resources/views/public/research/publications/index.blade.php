...
@section('content')
    ...
    <section class="bg-white pb-[80px] pt-[60px] font-hacen" dir="{{ $direction }}">
        <div class="container">
            <div class="research-filter-grid-publications mx-auto max-w-[1210px]">
                @foreach ([['faculties', 'facultyLabel'], ['publicationTypes', 'typeLabel'], ['years', 'yearLabel']] as [$key, $labelKey])
                    <label class="relative block">
                        <span class="sr-only">{{ $data['filters'][$labelKey] ?? '' }}</span>
                        <select class="h-[49px] w-full appearance-none rounded-[8px] border border-[#dde2ea] bg-white px-5 text-[15px] font-bold text-[#344054] shadow-[0_8px_18px_rgba(16,24,40,0.08)] outline-none transition focus:border-spu-blue">
                ...
                <label class="relative block">
                    ...
                    <input type="search" class="h-[49px] w-full rounded-[8px] border border-[#dde2ea] bg-white ps-12 pe-5 text-[13px] font-medium text-[#344054] shadow-[0_8px_18px_rgba(16,24,40,0.08)] outline-none transition placeholder:text-[#5b6473] focus:border-spu-blue" placeholder="{{ $data['filters']['searchPlaceholder'] ?? '' }}">
                </label>
            </div>
