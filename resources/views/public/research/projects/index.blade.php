...
@section('content')
    ...
    <section class="bg-white pb-[80px] pt-[60px] font-hacen" dir="{{ $direction }}">
        <div class="container">
            @php($activeFilters = $data['activeFilters'] ?? ['q' => '', 'status' => '', 'faculty' => '', 'theme' => '', 'page' => 1])
            <form method="GET" action="{{ ($isPreview ?? false) && isset($preview) ? '/'.$locale.'/preview' : '/'.$locale.'/research/projects' }}" class="research-filter-grid-projects mx-auto max-w-[1210px]">
                @if (($isPreview ?? false) && isset($preview))<input type="hidden" name="token" value="{{ $preview->token }}">@endif
                @foreach ([['statuses', 'statusLabel', 'status'], ['faculties', 'facultyLabel', 'faculty'], ['themes', 'themeLabel', 'theme']] as [$key, $labelKey, $inputName])
                    <label class="relative block"><span class="sr-only">{{ $data['filters'][$labelKey] ?? '' }}</span><select name="{{ $inputName }}" onchange="this.form.submit()" class="h-[49px] w-full appearance-none rounded-[8px] border border-[#dde2ea] bg-white px-5 text-[15px] font-bold text-[#344054] shadow-[0_8px_18px_rgba(16,24,40,0.08)] outline-none transition focus:border-spu-blue">
                        <option value="">{{ $data['filters'][$labelKey] ?? '' }}</option>
                        @foreach (($data['filters'][$key] ?? []) as $option)<option value="{{ $option['value'] ?? '' }}" @selected(($activeFilters[$inputName] ?? '') === (string) ($option['value'] ?? ''))>{{ $option['label'] ?? '' }}</option>@endforeach
                    </select><img src="/images/icon-chevron-down-outline.svg" alt="" class="pointer-events-none absolute right-5 top-1/2 h-3 w-3 -translate-y-1/2 rtl:left-5 rtl:right-auto" aria-hidden="true"></label>
                @endforeach
                <label class="relative block"><span class="sr-only">{{ $locale === 'ar' ? 'بحث' : 'Search' }}</span><img src="/images/icon-search-outline.svg" alt="" class="pointer-events-none absolute left-5 top-1/2 h-4 w-4 -translate-y-1/2 opacity-70 rtl:left-auto rtl:right-5" aria-hidden="true"><input name="q" value="{{ $activeFilters['q'] ?? '' }}" type="search" class="h-[49px] w-full rounded-[8px] border border-[#dde2ea] bg-white ps-12 pe-5 text-[13px] font-medium text-[#344054] shadow-[0_8px_18px_rgba(16,24,40,0.08)] outline-none transition placeholder:text-[#5b6473] focus:border-spu-blue" placeholder="{{ $data['filters']['searchPlaceholder'] ?? '' }}"></label>
            </form>
