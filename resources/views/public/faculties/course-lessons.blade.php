        ...
        @if (is_array($course))
            <div class="grid gap-8 lg:grid-cols-[0.78fr_1.22fr] lg:items-start">
                ...
                <div class="space-y-8">
                    ...
                    <section class="rounded-[8px] border border-slate-200 bg-white p-6 shadow-sm md:p-7">
                        <div class="mb-6 flex items-center justify-between gap-4"><h2 class="text-[24px] font-bold text-spu-blue">{{ $label('lessons') }}</h2><div class="flex flex-wrap gap-2"><a href="{{ $courseHref($course) }}" class="rounded-[2px] px-3 py-1 text-[11px] font-bold uppercase tracking-wide {{ $selectedType === 'all' ? 'bg-spu-blue text-white' : 'bg-slate-100 text-slate-500' }}">{{ $label('all') }}</a>@foreach ($availableTypes as $type)<a href="{{ $courseHref($course) }}&type={{ urlencode((string) $type) }}" class="rounded-[2px] px-3 py-1 text-[11px] font-bold uppercase tracking-wide {{ $selectedType === $type ? 'bg-spu-blue text-white' : 'bg-slate-100 text-slate-500' }}">{{ $lessonTypes[$type]['label'] ?? $type }}</a>@endforeach</div></div>
