...
<div class="font-hacen {{ $isAr ? 'rtl' : '' }}" dir="{{ $direction }}" data-study-plan data-locale="{{ $locale }}" data-faculty-slug="{{ $page->facultySlug }}" style="--accent-color: {{ $accent }}; --accent-color-15: {{ $accent }}26;">
        ...
        <div class="container">
            ...
            @else
                <div class="space-y-10">
                    ...
                    <div data-study-plan-viewport dir="ltr" class="relative w-full overflow-hidden border border-slate-200/50 bg-white shadow-[0_8px_40px_rgba(32,39,89,0.04)] sp-custom-scrollbar" style="height: 80vh;">
                        <div class="absolute right-4 top-4 z-50 flex flex-col gap-2 rtl:left-4 rtl:right-auto">
                            <button type="button" data-study-plan-zoom-in class="flex h-9 w-9 items-center justify-center rounded-lg border border-slate-200 bg-white text-lg font-black text-spu-blue shadow-md transition-colors hover:bg-slate-50" aria-label="Zoom in">+</button>
                            <button type="button" data-study-plan-zoom-out class="flex h-9 w-9 items-center justify-center rounded-lg border border-slate-200 bg-white text-lg font-black text-spu-blue shadow-md transition-colors hover:bg-slate-50" aria-label="Zoom out">−</button>
                            <button type="button" data-study-plan-fit class="flex h-9 w-9 items-center justify-center rounded-lg border border-slate-200 bg-white text-lg font-black text-spu-blue shadow-md transition-colors hover:bg-slate-50" aria-label="Fit to screen">⟲</button>
                        </div>
    ...
    <div data-course-modal class="fixed inset-0 z-[100] hidden items-center justify-center bg-black/40 p-4 backdrop-blur-sm">
        <div class="relative w-[90%] max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl" dir="{{ $direction }}">
            <button type="button" data-modal-close class="absolute right-3 top-3 text-slate-400 transition-colors hover:text-slate-600 rtl:left-3 rtl:right-auto" aria-label="{{ $label('close') }}"><svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg></button>
            ...
            <h3 data-modal-title class="text-[18px] font-black leading-tight text-spu-blue"></h3>
