    ...
    <section class="bg-white py-12 font-hacen md:py-16">
        <div class="container mx-auto px-6">
            <div class="mx-auto grid max-w-[1120px] gap-8 lg:grid-cols-[minmax(0,1fr)_360px]">
                <form method="POST" action="/{{ $locale }}/e-services/suggestions-complaints" enctype="multipart/form-data" class="rounded-2xl bg-gray-50 p-6 shadow-sm sm:p-8">
                    @csrf
                    ...
                    <div class="grid gap-5 sm:grid-cols-2">
                        <label class="block"><span class="text-xs font-bold uppercase tracking-wide text-gray-700">{{ __('public.full_name') }} *</span><input name="fullName" value="{{ old('fullName') }}" required aria-describedby="fullName-error" class="mt-1 w-full rounded-lg border border-gray-200 bg-white px-4 py-3 text-sm outline-none transition focus:border-spu-red focus:ring-2 focus:ring-spu-red/10">@error('fullName')<span id="fullName-error" class="mt-1 block text-xs font-bold text-red-700">{{ $message }}</span>@enderror</label>
                        ...
                        <label class="block"><span class="text-xs font-bold uppercase tracking-wide text-gray-700">{{ $locale === 'ar' ? 'رقم الهاتف' : 'Phone' }} *</span><input name="phone" type="tel" value="{{ old('phone') }}" required class="mt-1 w-full rounded-lg border border-gray-200 bg-white px-4 py-3 text-sm outline-none transition focus:border-spu-red focus:ring-2 focus:ring-spu-red/10">@error('phone')<span class="mt-1 block text-xs font-bold text-red-700">{{ $message }}</span>@enderror</label>
                        <label class="block"><span class="text-xs font-bold uppercase tracking-wide text-gray-700">{{ __('public.request_type') }} *</span><select name="requestType" required class="mt-1 w-full rounded-lg border border-gray-200 bg-white px-4 py-3 text-sm outline-none transition focus:border-spu-red focus:ring-2 focus:ring-spu-red/10">@foreach (($form['requestTypes'] ?? []) as $type)<option value="{{ $type['value'] ?? '' }}" @selected(old('requestType') === ($type['value'] ?? ''))>{{ $type['label'] ?? '' }}</option>@endforeach</select></label>
                        ...
                        <label class="block sm:col-span-2"><span class="text-xs font-bold uppercase tracking-wide text-gray-700">{{ $locale === 'ar' ? 'مرفق' : 'Attachment' }}</span><input name="attachment" type="file" accept=".pdf,.jpg,.jpeg,.png,application/pdf,image/jpeg,image/png" class="mt-1 w-full rounded-lg border border-gray-200 bg-white px-4 py-3 text-sm"><span class="mt-1 block text-xs text-gray-500">{{ $form['attachmentHelp'] ?? '' }}</span>@error('attachment')<span class="mt-1 block text-xs font-bold text-red-700">{{ $message }}</span>@enderror</label>
                        <label class="flex items-start gap-3 sm:col-span-2"><input name="consent" value="1" type="checkbox" required @checked(old('consent')) class="mt-1 h-4 w-4 rounded border-gray-300 text-spu-red focus:ring-spu-red"><span class="text-sm font-semibold leading-6 text-gray-700">{{ $form['consentLabel'] ?? '' }} *</span></label>
                    </div>
