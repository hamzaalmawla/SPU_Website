        ...
        <div class="section-header relative">
            ...
            <div class="section-header__controls flex gap-3 absolute top-0 z-50 rtl:left-0 ltr:right-0">
                @if ($section->payload->sectionAction)
                    <a href="{{ $section->payload->sectionAction->url }}" class="flex h-10 w-full max-w-[195px] items-center justify-center gap-3 rounded-[12px] bg-[#1e2652] text-center text-sm font-bold text-white transition-all hover:bg-opacity-90" @if ($section->payload->sectionAction->target) target="{{ $section->payload->sectionAction->target }}" rel="noreferrer" @endif>{{ $section->payload->sectionAction->label }}</a>
                @endif
