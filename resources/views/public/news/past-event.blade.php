        ...
        <div class="container max-w-[1100px]">
            ...
            @else
                <article class="overflow-hidden rounded-2xl bg-white shadow-sm">
                    <img src="{{ $event->imageUrl }}" alt="{{ $event->title }}" class="h-[360px] w-full object-cover">
                    <div class="p-7 md:p-10">
                        ...
                        @endif
                        @if ($event->gallery !== [])
                            <h2 class="mt-10 text-xl font-bold text-spu-blue">{{ $page['galleryLabel'] }}</h2><div class="mt-4 grid gap-4 sm:grid-cols-2">@foreach($event->gallery as $image)<a href="{{ $image }}" target="_blank" rel="noopener noreferrer"><img src="{{ $image }}" alt="{{ $event->title }}" loading="lazy" class="h-52 w-full rounded-lg object-cover"></a>@endforeach</div>
                        @endif
                    </div>
                </article>
            @endif
