...
@section('content')
    ...
    <section class="relative flex h-screen items-center justify-center overflow-hidden py-24">
        ...
        <div class="container relative top-26 z-10 text-center text-white">
            <h1 class="mb-6 text-[48px] font-bold uppercase tracking-[0.4em] text-spu-gold">{{ $isAr ? 'الأخبار' : 'NEWS' }}</h1>
            <p class="mb-8 text-[30px] font-bold leading-tight">{{ $pageDescription }}</p>
                ...
                <article class="group flex flex-col items-center gap-12 lg:flex-row">
                    ...
                    <div class="w-full lg:w-1/2">
                        <div class="mb-6 flex items-center gap-3 text-sm font-bold uppercase tracking-widest text-spu-red">
                            <span>{{ $featured->categoryLabel ?: ($isAr ? 'أخبار الجامعة' : 'University News') }}</span>
    ...
    <section id="last-news" class="bg-white py-20 font-hacen">
        <div class="container">
            <div class="mb-12 flex items-center justify-between gap-4">
                <h2 class="text-5xl font-bold text-[#202759]">{{ $isAr ? 'آخر الأخبار' : 'Last News' }}</h2>
                <a href="/{{ $locale }}/news/articles" class="rounded border border-slate-300 px-10 py-2 text-sm font-bold text-[#202759] transition-colors hover:bg-slate-50">{{ $isAr ? 'عرض الكل' : 'View All News' }}</a>
            </div>
                        ...
                        <a href="{{ $news->url ?? '#' }}" class="block">
                            <div class="relative w-full overflow-hidden {{ $loop->iteration === 4 ? 'h-[60%]' : 'h-[55%]' }}">
                                ...
                                <div class="absolute left-5 top-5 rtl:left-auto rtl:right-5">
                                    <span class="rounded-lg bg-white px-5 py-1.5 text-[12px] font-bold text-[#202759] shadow-sm">{{ $news->categoryLabel ?: ($isAr ? 'أخبار' : 'News') }}</span>
                                </div>
                            </div>
                        </a>
    ...
    <section class="bg-white py-24 font-hacen">
        <div class="container">
            <div class="mb-12 flex items-center justify-between gap-4">
                <h2 class="text-5xl font-bold text-[#202759]">{{ $isAr ? 'الإعلانات' : 'Announcements' }}</h2>
                <a href="/{{ $locale }}/news/articles?category=announcements" class="rounded border border-slate-300 px-8 py-2 text-sm font-bold text-[#202759] transition-colors hover:bg-slate-50">{{ $isAr ? 'عرض كافة الإعلانات' : 'View All Announcements' }}</a>
            </div>
            ...
            <div class="grid items-stretch gap-8 lg:grid-cols-12">
                <div id="events" class="lg:col-span-4">
                    <div class="flex h-full flex-col rounded-[12px] bg-[#1e2a5e] p-10 text-white">
                        <div class="mb-12 flex items-center justify-between border-b border-white/20 pb-6">
                            <h3 class="text-3xl font-bold">{{ $isAr ? 'الفعاليات القادمة' : 'Upcoming Events' }}</h3>
                        ...
                        <div class="relative flex-grow space-y-12 pb-10">
                            ...
                            @foreach ($events as $event)
                                <a href="{{ $event->url ?? '#' }}" class="relative block pl-10 rtl:pl-0 rtl:pr-10">
                                    ...
                                    <p class="text-[13px] uppercase tracking-wide text-white/50">{{ $event->categoryLabel ?: ($isAr ? 'أخبار الجامعة' : 'University News') }}</p>
                                </a>
                            @endforeach
                        </div>

                        <a href="/{{ $locale }}/news/articles" class="mt-auto block w-full rounded-lg border border-white py-4 text-center text-sm font-bold transition-colors hover:bg-white/10">{{ $isAr ? 'عرض تفاصيل كافة الفعاليات' : 'View All Events Details' }}</a>
                    </div>
                </div>
                ...
                <div id="announcements" class="lg:col-span-8">
                    <div class="h-full overflow-hidden rounded-[12px] border border-slate-200 bg-white">
                            ...
                            <article class="group flex gap-8 p-10 {{ ! $loop->last ? 'border-b border-slate-100' : '' }}">
                                ...
                                <div class="flex-grow">
                                    <div class="mb-3 flex items-start justify-between gap-6">
                                        ...
                                        <span class="whitespace-nowrap rounded-md bg-[#202759] px-4 py-1.5 text-[11px] font-bold uppercase tracking-widest text-white">{{ $isAr ? 'جديد' : 'New' }}</span>
                                    </div>
                        ...
                        @empty
                            <div class="p-10 text-slate-600">{{ $isAr ? 'لا توجد إعلانات منشورة حالياً.' : 'No announcements are currently published.' }}</div>
                        @endforelse
                    </div>
                </div>
            </div>
    ...
    <section class="bg-section bg-slate-50 py-24 font-hacen">
        <div class="container">
            <h2 class="mb-16 text-center text-3xl font-bold text-spu-blue">{{ $isAr ? 'استكشف المزيد' : 'Explore More' }}</h2>
            <div class="mx-auto grid max-w-5xl gap-8 md:grid-cols-2">
                <a id="media-gallery" href="/{{ $locale }}/news/articles" class="group relative flex flex-col items-center gap-6 overflow-hidden rounded-[32px] border border-slate-100 bg-white p-12 text-center text-spu-blue shadow-sm transition-all duration-500 hover:translate-y-[-8px] hover:bg-[#1e2a5e] hover:text-white hover:shadow-2xl">
                    ...
                    <div>
                        <h3 class="mb-3 text-3xl font-bold">{{ $isAr ? 'أرشيف الأخبار' : 'News Archive' }}</h3>
                        <p class="flex items-center justify-center gap-2 text-sm font-bold uppercase tracking-widest opacity-60"><span>{{ $isAr ? 'انقر للزيارة' : 'Visit Room' }}</span><span class="text-lg">→</span></p>
                    </div>
                </a>
                ...
                <a id="press-room" href="/{{ $locale }}/news/articles?category=announcements" class="group relative flex flex-col items-center gap-6 overflow-hidden rounded-[32px] border border-slate-100 bg-white p-12 text-center text-spu-blue shadow-sm transition-all duration-500 hover:translate-y-[-8px] hover:bg-[#1e2a5e] hover:text-white hover:shadow-2xl">
                    ...
                    <div>
                        <h3 class="mb-3 text-3xl font-bold">{{ $isAr ? 'الإعلانات' : 'Announcements' }}</h3>
                        <p class="flex items-center justify-center gap-2 text-sm font-bold uppercase tracking-widest opacity-60"><span>{{ $isAr ? 'انقر للزيارة' : 'Visit Room' }}</span><span class="text-lg">→</span></p>
                    </div>
                </a>
            </div>
