...
@section('content')
    ...
    <section class="bg-white py-12 font-hacen">
        <div class="container">
            <div class="text-center"><p class="text-[11px] font-bold uppercase tracking-[0.16em] text-spu-blue/55">{{ $content['highlights']['eyebrow'] ?? '' }}</p><h2 class="mt-2 text-[24px] font-bold leading-tight text-spu-blue md:text-[30px]">{{ $content['highlights']['title'] ?? '' }}</h2><p class="mx-auto mt-3 w-full text-sm leading-6 text-spu-blue/60 md:w-[56%]">{{ $content['highlights']['summary'] ?? '' }}</p></div>
            <div class="mt-8 grid gap-4 lg:grid-cols-2">
                @foreach (($content['highlights']['items'] ?? []) as $item)
                    @php($highlightUrl = $item['href'] ?? null)
                    @if ($highlightUrl && $highlightUrl !== '#')<a href="{{ $highlightUrl }}" class="group relative min-h-[210px] overflow-hidden rounded-[8px] bg-spu-blue text-white shadow-[0_16px_42px_rgba(32,39,89,0.14)] {{ ($item['featured'] ?? false) ? 'lg:row-span-2 lg:min-h-[440px]' : '' }}">@else<div class="group relative min-h-[210px] overflow-hidden rounded-[8px] bg-spu-blue text-white shadow-[0_16px_42px_rgba(32,39,89,0.14)] {{ ($item['featured'] ?? false) ? 'lg:row-span-2 lg:min-h-[440px]' : '' }}">@endif
                        <img src="{{ $item['image'] ?? '' }}" alt="{{ $item['imageAlt'] ?? '' }}" class="absolute inset-0 h-full w-full object-cover transition duration-700 group-hover:scale-105"><div class="absolute inset-0 bg-gradient-to-t from-spu-blue/95 via-spu-blue/45 to-transparent"></div><div class="absolute inset-x-0 bottom-0 p-5 md:p-7"><h3 class="text-[22px] font-bold leading-tight {{ ($item['featured'] ?? false) ? 'md:text-[28px]' : 'md:text-[24px]' }}">{{ $item['title'] ?? '' }}</h3><p class="mt-2 text-sm leading-6 text-white/82">{{ $item['summary'] ?? '' }}</p><span class="mt-4 inline-flex items-center gap-2 rounded-[4px] bg-spu-red px-4 py-2 text-[11px] font-bold transition group-hover:bg-white group-hover:text-spu-blue"><span>{{ $item['label'] ?? '' }}</span><img src="/images/icon-arrow-right-outline.svg" alt="" class="h-3 w-3 brightness-0 invert rtl:rotate-180" aria-hidden="true"></span></div>
                    @if ($highlightUrl && $highlightUrl !== '#')</a>@else</div>@endif
                @endforeach
            </div>
        </div>
    </section>

    <section id="facilities" class="bg-[#f4f8fd] py-14 font-hacen">
        <div class="container"><div class="text-center"><p class="text-[11px] font-bold uppercase tracking-[0.16em] text-spu-blue/55">{{ $content['facilities']['eyebrow'] ?? '' }}</p><h2 class="mt-2 text-[24px] font-bold leading-tight text-spu-blue md:text-[30px]">{{ $content['facilities']['title'] ?? '' }}</h2><p class="mx-auto mt-3 w-full text-sm leading-6 text-spu-blue/60 md:w-[56%]">{{ $content['facilities']['summary'] ?? '' }}</p></div>
            <div class="mt-8 grid gap-5 md:grid-cols-3">
                @foreach (($content['facilities']['items'] ?? []) as $item)
                    @php($facilityUrl = $item['href'] ?? null)
                    @if ($facilityUrl && $facilityUrl !== '#')<a href="{{ $facilityUrl }}" class="group overflow-hidden rounded-[8px] border border-spu-blue/10 bg-white shadow-[0_12px_30px_rgba(32,39,89,0.08)] transition hover:-translate-y-1 hover:shadow-[0_18px_42px_rgba(32,39,89,0.14)]">@else<article class="group overflow-hidden rounded-[8px] border border-spu-blue/10 bg-white shadow-[0_12px_30px_rgba(32,39,89,0.08)]">@endif
                        <div class="relative h-[150px] overflow-hidden"><img src="{{ $item['image'] ?? '' }}" alt="{{ $item['title'] ?? '' }}" class="h-full w-full object-cover transition duration-700 group-hover:scale-105"><div class="absolute inset-0 bg-spu-blue/10"></div></div><div class="p-5 text-center"><div class="mx-auto grid h-12 w-12 place-items-center rounded-[8px] bg-spu-blue/5"><img src="{{ $item['icon'] ?? '' }}" alt="" class="h-6 w-6" aria-hidden="true"></div><h3 class="mt-4 text-[18px] font-bold text-spu-blue">{{ $item['title'] ?? '' }}</h3><p class="mt-2 text-sm leading-6 text-spu-blue/58">{{ $item['summary'] ?? '' }}</p>@if ($facilityUrl && $facilityUrl !== '#')<span class="mt-4 inline-flex items-center justify-center gap-2 text-[11px] font-bold uppercase tracking-[0.08em] text-spu-red"><span>{{ $content['facilities']['detailsLabel'] ?? '' }}</span><img src="/images/icon-arrow-right-outline.svg" alt="" class="h-3 w-3 rtl:rotate-180" aria-hidden="true"></span>@endif</div>
                    @if ($facilityUrl && $facilityUrl !== '#')</a>@else</article>@endif
                @endforeach
            </div>
        </div>
    </section>
