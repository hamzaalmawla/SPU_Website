...
@section('content')
        ...
        <div class="bg-white font-hacen" dir="{{ $direction }}" data-page-name="admissions-{{ $slug }}" data-page-content x-data="admissionsTabs()" data-active-tab="{{ $activeTab['id'] ?? '' }}">
            ...
            <section class="bg-white pb-28 pt-11">
                <div class="container mx-auto px-6">
                    ...
                    @foreach (($section['tabs'] ?? []) as $tab)
                        <div id="requirements-panel-{{ $tab['id'] ?? '' }}" role="tabpanel" aria-labelledby="requirements-tab-{{ $tab['id'] ?? '' }}" class="mx-auto mt-8 max-w-[1010px] space-y-16" data-tab-panel="{{ $tab['id'] ?? '' }}" x-show="isActive($el.dataset.tabPanel)" x-bind:hidden="!isActive($el.dataset.tabPanel)" {{ $loop->first ? '' : 'hidden' }}>
        ...
        <div class="bg-white font-hacen" dir="{{ $direction }}" data-page-name="admissions-{{ $slug }}" data-page-content x-data="admissionsTuition()">
            ...
            <section class="bg-white pb-24 pt-12">
                <div class="container mx-auto max-w-[1060px] px-6">
                    ...
                    @endphp
                    @if (! empty($section['feeRows']))<div class="flex flex-col gap-5 md:flex-row">
                    ...
                    </tbody></table></div>@else<div class="rounded-xl border border-spu-blue/10 bg-spu-blue/[0.04] p-6 text-sm font-bold leading-7 text-slate-700">{{ $section['availabilityGuidance'] ?? '' }}</div>@endif</section>

                    <section class="mt-16"><h2 class="mb-8 text-[26px] font-black leading-tight text-spu-blue">{{ $section['paymentTitle'] ?? '' }}</h2>@if (! empty($section['methods']))<div class="grid gap-10 md:grid-cols-2">
                        ...
                        @endforeach
                    </div>@else<div class="rounded-xl border border-spu-red/15 bg-red-50 p-6 text-sm font-bold leading-7 text-slate-700">{{ $section['paymentGuidance'] ?? '' }}</div>@endif</section>
    ...
    @elseif ($slug === 'how-to-apply')
        <div class="admissions-design" dir="{{ $direction }}" data-page-name="admissions-{{ $slug }}" data-page-content>
            ...
            <section class="admissions-shell admissions-feature-cards" aria-label="{{ $locale === 'ar' ? 'أبرز محطات رحلة القبول' : 'Admissions journey highlights' }}">
                    ...
                    @foreach (($section['steps'] ?? []) as $step)
                        <article class="admissions-timeline-step {{ $loop->last ? 'admissions-timeline-step--final' : '' }}">
                            ...
                            <div class="admissions-step-card">
                                <h3>{{ $step['title'] ?? '' }}</h3>
                                <p>{{ $step['desc'] ?? '' }}</p>
                                @if (! empty($step['href']) && ! empty($step['cta']))<a class="admissions-step-card__button" href="{{ $step['href'] }}">{{ $step['cta'] }}</a>@endif
                            </div>
                        </article>
                    @endforeach
                </div>
                            ...
                            <div class="admissions-accordion">
                                @foreach (($group['items'] ?? []) as $item)
                                    ...
                                    <article class="admissions-accordion__item {{ $loop->parent->first && $loop->first ? 'is-open' : '' }}" data-faq-item data-category="{{ $categoryId }}" data-index="{{ $loop->index }}" data-search="{{ $searchText }}" x-show="itemVisible($el)" x-bind:class="accordionItemClass($el)">
                                        <button type="button" id="faq-question-{{ $categoryId }}-{{ $loop->index }}" class="admissions-accordion__button" data-category="{{ $categoryId }}" data-index="{{ $loop->index }}" aria-controls="faq-answer-{{ $categoryId }}-{{ $loop->index }}" aria-expanded="{{ $loop->parent->first && $loop->first ? 'true' : 'false' }}" x-bind:aria-expanded="isOpen($el)" x-on:click="toggleAccordion($event)">
                                        ...
                                        <div id="faq-answer-{{ $categoryId }}-{{ $loop->index }}" role="region" aria-labelledby="faq-question-{{ $categoryId }}-{{ $loop->index }}" class="admissions-accordion__answer" data-category="{{ $categoryId }}" data-index="{{ $loop->index }}" x-show="isOpen($el)" x-bind:hidden="!isOpen($el)" {{ $loop->parent->first && $loop->first ? '' : 'hidden' }}><p>{{ $item['a'] ?? '' }}</p></div>
                                    </article>
                                @endforeach
                            </div>
                        </section>
    ...
    @elseif ($slug === 'calendar')
        <div class="admissions-design" dir="{{ $direction }}" data-page-name="admissions-{{ $slug }}" data-page-content>
            ...
            <section class="admissions-calendar-page"><div class="admissions-shell">
                <div class="admissions-stat-cards" aria-label="{{ $locale === 'ar' ? 'أبرز معلومات التقويم' : 'Calendar highlights' }}">
                ...
                @if (! empty($section['deadlines']))<section class="admissions-deadlines" aria-labelledby="essential-deadlines-title"><h2 id="essential-deadlines-title" class="admissions-page-title">{{ $section['deadlinesTitle'] ?? '' }}</h2><div class="admissions-deadline-grid">
                ...
                @if (! empty($section['semesters']))<section aria-labelledby="detailed-timeline-title"><h2 id="detailed-timeline-title" class="admissions-page-title">{{ $section['timelineTitle'] ?? '' }}</h2><div class="admissions-calendar-timeline-card">
                ...
                @if (empty($section['deadlines']) && empty($section['semesters']))<div class="admissions-notice-card"><div><h3>{{ $section['timelineTitle'] ?? '' }}</h3><p>{{ $section['scheduleGuidance'] ?? '' }}</p></div></div>@endif
                @if (! empty($section['download']['href']) && ! empty($section['download']['mediaId']))<article class="admissions-download-card"><span class="admissions-download-icon" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M6 2h9l5 5v20H6z"></path><path d="M15 2v6h5"></path><rect x="8" y="12" width="9" height="7" rx="1"></rect><path d="M10 14h2.5a1.5 1.5 0 0 1 0 3H10v-3Z"></path></svg></span><div><h3>{{ $section['download']['title'] ?? '' }}</h3><p>{{ $section['download']['desc'] ?? '' }}</p></div><a class="admissions-download-button" href="{{ $section['download']['href'] }}" download><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><path d="M12 3v12"></path><path d="m7 10 5 5 5-5"></path><path d="M5 20h14"></path></svg><span>{{ $section['download']['button'] ?? '' }}</span></a></article>@endif
    ...
    @elseif ($slug === 'documents')
        @php
            ...
            $documentTabQueries = [
                ...
                'granted' => 'university-documents',
                'studySystem' => 'study-system',
                'warnings' => 'academic-warnings',
            ];
            $requestedDocumentTab = request()->query('tab');
            $activeDocumentTab = array_search($requestedDocumentTab, $documentTabQueries, true);
            $activeDocumentTab = is_string($activeDocumentTab) ? $activeDocumentTab : 'checklist';
        @endphp
        <div class="bg-white font-hacen" dir="{{ $direction }}" data-page-name="admissions-{{ $slug }}" data-page-content x-data="admissionsDocuments()" data-active-tab="{{ $activeDocumentTab }}" data-active-sub-tab="{{ $firstSubTab['id'] ?? 'freshman' }}">
            <section class="bg-white">
                <div class="relative flex h-[370px] items-center justify-center overflow-hidden lg:h-[400px]">
                    ...
                    <div class="container relative z-10 mx-auto px-6 text-center text-white">
                        ...
                        @if (! empty($section['applyUrl']) || ! empty($section['requestInfoUrl']))<div class="mt-7 flex flex-wrap items-center justify-center gap-3">@if (! empty($section['applyUrl']))<a href="{{ $section['applyUrl'] }}" class="inline-flex h-10 min-w-[136px] items-center justify-center rounded-md bg-spu-red px-8 text-xs font-black uppercase tracking-wide text-white transition hover:bg-spu-blue">{{ $section['applyLabel'] ?? '' }}</a>@endif @if (! empty($section['requestInfoUrl']))<a href="{{ $section['requestInfoUrl'] }}" class="inline-flex h-10 min-w-[136px] items-center justify-center rounded-md border border-white bg-white/5 px-8 text-xs font-black text-white transition hover:bg-white hover:text-spu-blue">{{ $section['requestInfoLabel'] ?? '' }}</a>@endif</div>@endif
                    </div>
                </div>
            </section>

            <section class="border-b border-slate-200 bg-white"><div class="container mx-auto px-6"><div class="mx-auto flex max-w-[900px] items-center justify-center overflow-x-auto text-center text-xs font-black tracking-wide text-slate-700" role="tablist" aria-label="{{ $title }}" data-documents-tablist>
                @foreach (($section['tabs'] ?? []) as $tab)
                    @php
                        $tabId = $tab['id'] ?? '';
                    @endphp
            ...
            @foreach (($section['tabs'] ?? []) as $tab)
                @if (($tab['id'] ?? '') === 'checklist')
                    <section id="documents-panel-checklist" class="bg-white py-16 md:py-20" role="tabpanel" aria-labelledby="documents-tab-checklist" data-documents-panel="checklist" x-show="isTab('checklist')" x-bind:hidden="!isTab('checklist')" {{ $activeDocumentTab === 'checklist' ? '' : 'hidden' }}><div class="container mx-auto px-6">
                        ...
                        @foreach (($tab['subTabs'] ?? []) as $sub)
                            <div id="documents-sub-panel-{{ $sub['id'] ?? '' }}" class="mx-auto max-w-[1010px]" role="tabpanel" aria-labelledby="documents-sub-tab-{{ $sub['id'] ?? '' }}" data-sub-tab-panel="{{ $sub['id'] ?? '' }}" x-show="isSubTab('{{ $sub['id'] ?? '' }}')">
                                ...
                                @if (! empty($sub['download']['href']) && ! empty($sub['download']['mediaId']))<div class="admissions-download-card mt-10"><div class="admissions-download-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><path d="m7 10 5 5 5-5"></path><path d="M12 15V3"></path></svg></div><div><h3>{{ $section['downloadLabel'] ?? '' }} — {{ $sub['label'] ?? '' }}</h3><p>{{ $sub['download']['size'] ?? '' }}</p></div><a href="{{ $sub['download']['href'] }}" class="admissions-download-button" download><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" aria-hidden="true"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><path d="m7 10 5 5 5-5"></path><path d="M12 15V3"></path></svg><span>{{ $section['downloadLabel'] ?? '' }}</span></a></div>@endif
                            </div>
                        @endforeach
                    </div></section>
                @elseif (($tab['id'] ?? '') === 'granted')
                    <section id="documents-panel-granted" class="bg-white py-16 md:py-20" role="tabpanel" aria-labelledby="documents-tab-granted" data-documents-panel="granted" x-show="isTab('granted')" x-bind:hidden="!isTab('granted')" {{ $activeDocumentTab === 'granted' ? '' : 'hidden' }}><div class="container mx-auto px-6"><div class="mx-auto max-w-[1010px]"><p class="mb-10 text-center text-sm font-bold leading-7 text-slate-700">{{ $tab['intro'] ?? '' }}</p><div class="cms-grid-cards gap-6">
    ...
    @elseif ($slug === 'transfer')
        ...
        <div class="bg-white font-hacen" dir="{{ $direction }}" data-page-name="admissions-{{ $slug }}" data-page-content x-data="admissionsTabs()" data-active-tab="{{ $activeTab['id'] ?? '' }}">
            ...
            <section class="bg-white pb-28 pt-20"><div class="container mx-auto max-w-[1060px] px-6">
                <div class="mx-auto flex w-fit max-w-full items-center overflow-hidden rounded-xl border border-slate-200 bg-white p-1 shadow-[0_8px_24px_rgba(32,39,89,0.10)]" role="tablist" aria-label="{{ $title }}">
                    @foreach (($section['tabs'] ?? []) as $tab)
                        <button type="button" id="transfer-tab-{{ $tab['id'] ?? '' }}" role="tab" aria-controls="transfer-panel-{{ $tab['id'] ?? '' }}" class="min-w-[180px] rounded-lg px-5 py-4 text-base font-black transition {{ $loop->first ? 'bg-spu-red text-white' : 'text-slate-600 hover:text-spu-red' }}" data-tab="{{ $tab['id'] ?? '' }}" x-bind:class="pillButtonClass($el)" aria-selected="{{ $loop->first ? 'true' : 'false' }}" x-bind:aria-selected="isActive($el.dataset.tab)" tabindex="{{ $loop->first ? '0' : '-1' }}" x-bind:tabindex="tabIndex($el.dataset.tab)" x-on:click="setActiveTab($event)" x-on:keydown="moveTab($event)">{{ $tab['label'] ?? '' }}</button>
                    @endforeach
                </div>
                @foreach (($section['tabs'] ?? []) as $tab)
                    <div id="transfer-panel-{{ $tab['id'] ?? '' }}" role="tabpanel" aria-labelledby="transfer-tab-{{ $tab['id'] ?? '' }}" data-tab-panel="{{ $tab['id'] ?? '' }}" x-show="isActive($el.dataset.tabPanel)" x-bind:hidden="!isActive($el.dataset.tabPanel)" {{ $loop->first ? '' : 'hidden' }}>
