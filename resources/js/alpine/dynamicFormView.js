...
export function createDynamicFormView() {
    return {
        ...
        fieldId(field) { return this.store.fieldId(field.name); },
        errorId(field) { return `${this.fieldId(field)}-error`; },
        describedBy(field) { return this.store.errors[field.name] ? this.errorId(field) : null; },
        invalid(field) { return this.store.errors[field.name] ? 'true' : 'false'; },
